...
def read_image(fp, target_size, name=None):
    """
    ...
    return tf.image.resize(image, target_size, 'bicubic', name=name)
...
def get_dataset(images_dir, **kwargs):
    return tf.data.Dataset.list_files(path.join(images_dir, '*')).map(partial(prepare_example, **kwargs))
